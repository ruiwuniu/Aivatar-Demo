...
- `src/App.tsx`
  ...
  - Autonomous Coffee Machine selection must remain blocked while cooking is queued or active. `cookingInteractionInFlight` and the pending/active world-interaction guards prevent idle-life `brew` selection from interrupting a Gas Range with Oven cooking trip or pan-toss sequence; keep this guard when adding new autonomous appliance behaviors.
...
- Park / hilltop side room
  ...
  - All six raw/cooked fish item names and descriptions are localized in both `zh-Hant` and `zh-Hans` through `src/i18n.ts`; keep localization exhaustive when adding or renaming fish species.
  ...
  - `src/park/parkWeather.ts` owns deterministic local-week weather. Each week schedules exactly two distinct rainy days from the save/slot seed; rain lasts from 10 minutes to 6 hours, consecutive events retain at least 10 clear minutes, moderate/heavy runs last at least 10 minutes, and storm runs last at least 5 minutes. The runtime progresses through clear, cloud gathering, rain intensity keyframes, tapering, and clearing with smooth interpolation. Debug preview modes are non-persistent and include static intensity stages plus an accelerated complete cycle.
  - `src/park/parkWeatherRenderer.ts` owns storm clouds, full-backdrop sea/cliff haze, back/front rain fields, pond ripples, grass splashes, and time-of-day weather grading. The accepted `public/park/hilltop-park-weather-backdrop-mask.png` follows the foreground cliff edge and covers the full sky/sea backdrop; keep its horizon aligned and do not replace it with a coarse runtime polygon. Night rain uses darker blue-gray colors. Rain line sprites and deterministic particle seeds are cached, grass splashes use a cropped buffer, and surface effects update on bounded intervals so visual tuning does not reintroduce per-frame full-scene construction work.
  - `src/park/parkWeatherAudio.ts` owns the three continuously crossfaded rain layers and near/medium/distant thunder candidates under `public/audio/weather-samples/`. Rain intensity changes use smoothed gains without stopping/restarting loops. Thunder is storm-only, randomized with explicit silence gaps, and must not overlap into an indistinct continuous rumble. Weather audio follows the independent park ambience volume path and releases all media/AudioContext resources on park unmount.
  ...
  - `src/park/parkReferenceLayers.ts` owns source-image loading and the exact sea, shoreline-foam, pond, land, dock, vegetation, cliff-fog, grass-ripple, weather-backdrop, and collision masks. Preserve the original PNG assets under `public/park/`; do not paint over or destructively edit them to create motion. Pond masks now carry their audited tight bounds, and construction canvases use those bounds where possible instead of retaining unnecessary scene-size buffers. Fog segment edges use feathered authored alpha, and the stationary cliff-fog underlay fills gaps exposed by moving fog without revealing the lower painted layer. Grass ripples exclude navigation colliders plus contour-derived rock/shrub occluders. Derived cloud, fog, grass, foam, sparkle, reflection, rain, haze, and pond effects must stay within their intended masks so they cannot cover grass obstacles, cliffs, islands, docks, reeds, lily pads, or other foreground art.
  ...
  - `src/park/parkRenderer.ts` owns render order and presentation: time-of-day sky/sea grading, horizon tint, stars, looping clouds, weather sky/haze/rain passes, sea sparkles, independently breathing shoreline foam, cliff fog, grass highlights/splashes, park objects and their time-aware shadows, avatar/fishing overlays, pond animation, and weather pond ripples. Keep dynamic sky/cloud/weather layers separate from the ground reference art so dawn, noon, sunset, night, and rain can transition without replacing accepted terrain geometry.
...
- `src/game/renderScene.ts`
  ...
  - Oil Easel canvas occlusion at the bed foot is resolved against the original bed and all related bed skins. When an avatar is sleeping/on the bed, the easel canvas must keep its normal depth relationship without being incorrectly covered by the sleep blanket or full bed foreground pass.
  ...
  - During the front-facing fishing cast, `cute-crayfish` uses both of its existing claws at a compact chest-level rod grip. Do not draw an extra claw on the rod or stretch either arm to reach a distant generic hand anchor; the rod geometry and claw-arm pose must share the same two grip points.
...
- `src-tauri/tauri.conf.json`
  ...
  - Current desktop release metadata is `0.4.4` across `package.json`, `package-lock.json`, `src-tauri/Cargo.toml`, `src-tauri/Cargo.lock`, and `src-tauri/tauri.conf.json`. Release tag `v0.4.4` publishes the universal macOS DMG `Aivatar_0.4.4_universal.dmg`, Windows NSIS installer `Aivatar_0.4.4_x64-setup.exe`, Windows MSI installer `Aivatar_0.4.4_x64_en-US.msi`, and checksum manifests generated by `.github/workflows/release-macos.yml` and `.github/workflows/release-windows.yml`.
