...
- `src/App.tsx`
  ...
  - Includes a saved global SFX volume slider stored under `aivatar.audioVolume.v1`. The slider controls current app sound effects, with Game Console audio playing at half the global SFX volume.
  - Owns runtime audio playback for animation-synchronized SFX: the built-in Terminal keyboard loop follows the placed Terminal monitor's coding/thinking screen-and-keyboard animation, while Game Console play audio starts only once the avatar is in real `play` behavior and the targeted console animation is active. Game Console audio randomly chooses one track per play-animation session from the `public/audio/game-console-*` pool and does not switch tracks mid-play.
...
- `public/audio/`
  - Stores bundled CC0 audio assets used by the frontend.
  - `keyboard-typing-loop.wav` comes from OpenGameArt Keyboard Soundpack #1 and loops while the Terminal monitor animation is active.
  ...
  - `README.md` records source URLs, authors, original filenames, and CC0 license notes for each bundled audio file.
...
- Animation-synchronized audio MVP:
  - Terminal keyboard typing loop plays with the placed Terminal monitor's coding/thinking animation.
