...
- `src/game/renderScene.ts`
  ...
  - Accepts the current UI theme so the canvas presentation can harmonize with Classic, Terminal, Amber, and Arcade Cabinet app-shell skins. `App.tsx` passes `arcade-cabinet` through `uiThemeForScene()` so the canvas room backdrop, scanline/status colors, progress bars, and bubbles can use an Arcade-specific palette while CSS carries the shell/frame skin.
  ...
  - The File Cabinet renderer uses `src/game/fileCabinetSprites.ts`, a `56 x 77` code-embedded palette/row matrix sprite rendered at `item.x - 6`, `item.y - 11`. It preserves the front-facing orthographic cabinet rule: the top plane is rectangular with equal-width front/back edges, deeper top depth, right-side shading, clean feet, and a tidy bottom contact shadow. Highlight bounds, foreground overlay bounds, and glow blockers use the same sprite envelope.
...
- The UI skins are currently CSS/canvas theme layers rather than a full design-token system. New UI components need explicit Classic/Terminal/Amber/Arcade Cabinet theme QA so Classic Windows 98 controls, Terminal/Amber colors, Arcade cabinet/CRT framing, the Arcade scene-panel top bright strip, expanded panels, custom progress bars, disabled states, Decor thumbnails, checkboxes, range sliders, Room Visit toast/dialog/list controls, and canvas overlays/bubbles stay coherent.
