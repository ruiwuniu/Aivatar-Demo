...
Use `http://127.0.0.1:1424/` for the current furniture-skin preview. This
origin has separate `localStorage` from `1420`, `1421`, `1422`, and `1423`.
...
- `src/App.tsx`
  ...
  - First furniture skin pass is Industrial Bed Skin for the base bed. Purchased skins can be applied without entering placement mode and do not affect furniture placement, collision, pathfinding, or interaction targets.
...
- `public/config/aivatar.config.json`
  ...
  - Furniture skin shop content currently includes Industrial Bed Skin, priced at `240` bits, with `tags: ["furniture-skin", "bed"]` and `targetFurnitureId: "bed"`. Runtime content is duplicated in `public/config/aivatar.config.json` and the `src/data/defaultContent.ts` fallback.
...
- Furniture Skin shop category:
  - First implemented skin is Industrial Bed Skin for the base bed.
...
6. Expand the furniture skin system beyond the first bed skin to Desk, Dining Table, Fridge, and File Cabinet, keeping skins visual-only unless a future skin explicitly changes dimensions/collision. Add screenshot/runtime QA for Furniture Skins: shop category visibility, purchased/apply/applied states, `activeFurnitureSkinIds` save/load, default bed fallback, Industrial Bed Skin normal rendering, and sleep blanket overlay.
