...
- `src/App.tsx`
  ...
  - Current character-preview worktree has the Debug card temporarily enabled with `SHOW_DEBUG_CARD = true` in `src/App.tsx` for visual QA. Decide whether to restore the default hidden/collapsed behavior before packaging or merging this work.
...
- `src/game/renderScene.ts`
  ...
  - Accepts the current UI theme so the canvas presentation can harmonize with Classic, Terminal, Amber, and Arcade Cabinet app-shell skins. `App.tsx` currently maps `arcade-cabinet` through `uiThemeForScene()` to the Terminal-style canvas palette, while the Arcade Cabinet identity is carried primarily by the CSS app-shell/scene-frame skin.
  ...
  - Renders floor surfaces through two paths: surfaces present in `FLOOR_SURFACE_SPRITE_DATA` use full `328 x 174` palette-plus-row matrix sprites from `src/game/floorSurfaceSprites.ts`, while remaining surfaces use procedural Canvas drawing. Current matrix floor sprites are Honey Plank, Checker Tile Floor, Polished Cement Floor, and Tatami Mat Floor.
  - Checker Tile Floor uses a clean black/white tile sprite with grout seams, beveled highlights, and sparse scuffs instead of heavy cracks. Polished Cement Floor uses a seamless single-slab smooth cement sprite with subtle clouding, sparse low-contrast pores, and soft gloss highlights. Industrial Metal Floor remains a procedural shaded plate/rivet floor. Gray Tech Floor remains a procedural matte composite floor with one offset horizontal plus one offset vertical blue LED strip and a late glow overlay. Tatami Mat Floor uses a green-bound woven straw matrix sprite.
...
- Floor surface sprites in `src/game/floorSurfaceSprites.ts` are code-embedded palette/row matrices, not external spritesheet/atlas assets. Current matrix floor surfaces are Honey Plank, Checker Tile Floor, Polished Cement Floor, and Tatami Mat Floor; they keep the same Decor surface ids, pricing, save-state fields, and floor interaction behavior as the content definitions.
- Digital Wall Clock, Sky Sentinel Poster, transparent glass Coffee Cup with slow animated steam, Cozy Rug, Morph Blob Rug, Blue Persian Rug, Switch-style Game Console, Coffee Machine, Oil Easel, File Cabinet, dynamic City Night Window, Ocean Window, Cyberpunk City Window, Purple Bubble Wallpaper, Exposed Red Brick Wallpaper, Pink Sakura Wallpaper, Warm Ivory Wallpaper, White Tech Wallpaper, Industrial Metal Floor, Gray Tech Floor, and recent fridge/coffee-machine/file-cabinet/easel art are still programmatic canvas assets rather than spritesheet/atlas assets.
...
- Shop and inventory item thumbnails now prefer the `public/icons/item-icons-arcade-a.png` `16 x 16` sprite for mapped item ids, while unmapped items and Decor wall/floor surface thumbnails remain lightweight CSS/DOM previews. These UI affordances can still drift from canvas art until the asset pipeline is unified; current item buttons align the thumbnail and quantity/price to opposite sides.
- The UI skins are currently CSS/canvas theme layers rather than a full design-token system. New UI components need explicit Classic/Terminal/Amber/Arcade Cabinet theme QA so Classic Windows 98 controls, Terminal/Amber colors, Arcade cabinet/CRT framing, expanded panels, custom progress bars, disabled states, Decor thumbnails, checkboxes, range sliders, and canvas overlays stay coherent.
...
- Translate the grid into a palette plus fixed row matrix in `src/game/renderScene.ts`, then render contiguous same-color runs with `drawPixelRect`. Use `.` or an equivalent token for transparent pixels.
