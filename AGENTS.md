...
- `src/App.tsx`
  ...
  - Owns the Record Player BGM MVP. The current track list includes the programmatic Web Audio `Pixel Parlor` loop plus bundled CC0 tracks `Bach BWV 577` and `Cyberpunk Moonlight`. Record Player playback is driven by an independent `activeRecordPlayerId` rather than by keeping the avatar in `music` behavior: after the avatar reaches and starts the Record Player, it returns to `idle` and can continue other activities while the selected Record Player keeps playing. BGM stops when the playing Record Player id is cleared, the placed Record Player disappears, BGM volume is muted, audio is not unlocked, the user chooses `Stop music` from the Record Player context menu, or the autonomous stop check succeeds. When the avatar starts music autonomously, it chooses a track itself, preferring a different track from the current one.
  ...
  - Terminal UI skin currently covers the side-panel shell, status header/card, language/theme buttons, Sound, Growth, Agent Sessions, Task Cabinet, CLI Launcher, Debug, stats grid, Decor controls, Inventory/Shop text, Asset Studio locked entry, expanded submenu cards, scene right-click context menus, custom context meters, collapsed room HUD overlays, and common button/input states.
  - Chinese UI typography has a local CSS font-face alias, `Aivatar CJK Serif`, that uses local `Noto Serif TC`, `Noto Serif SC`, or `Noto Serif HK` only for CJK unicode ranges. This lets Chinese DOM UI use the serif look while English text keeps the original Latin/monospace fallback stack.
...
- Sound settings live in a collapsible Sound side-panel card. Global SFX volume persists per browser/Tauri origin under `aivatar.audioVolume.v1`.
...
- Animation-synchronized audio MVP:
  ...
  - Record Player BGM supports a small track list: the programmatic Web Audio `Pixel Parlor` loop and bundled CC0 audio tracks `Bach BWV 577` and `Cyberpunk Moonlight`. Playback follows the independent `activeRecordPlayerId` state, so it can continue after the avatar starts the Record Player and leaves to do other activities. It is governed by the separate BGM volume, can be stopped from the Record Player right-click menu, and can be autonomously stopped by the avatar after it has played for a while.
- Record Player MVP:
  ...
  - Right-clicking the Record Player exposes in-context BGM track selection, BGM volume, and a `Stop music` button when that specific Record Player is playing.
  ...
  - After at least 45 seconds of playback, the avatar can autonomously stop BGM on a 60-second check with an 8% chance per check, as long as no high-priority agent state, blocking interaction, or Task Cabinet visual flow is active.
...
- UI theme preference is also browser-origin scoped under `aivatar.uiTheme.v1`. A Terminal skin choice made at `http://127.0.0.1:1421/` will not automatically apply to `http://localhost:1420/`. Task Cabinet cards, schedules, buttons, paths, status text, compact card headers, stats grid positioning, expand-button sizing/alignment, Sound controls, and scene right-click context menus now have explicit Terminal and Amber coverage where needed, but the skin system is still selector-based rather than token-driven.
...
14. Continue refining the UI skin system toward reusable theme tokens so future panels inherit Classic/Terminal/Amber colors without one-off selector patches.
