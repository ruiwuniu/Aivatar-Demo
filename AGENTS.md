...
- `scripts/codex-status-bridge.mjs`
  ...
  - Treats sessions as stale after `AIVATAR_SESSION_STALE_MS` milliseconds, defaulting to 48 hours; stale sessions stay visible in the list but no longer block interaction or drive the main avatar state.
...
- `scripts/codex-session-discovery.mjs`
  ...
  - Only considers rollout files modified within `AIVATAR_DISCOVERY_ACTIVE_MS`, defaulting to 48 hours, so old chat history outside the expiry window is not eagerly connected.
