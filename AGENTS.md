...
- `src/game/simulation.ts`
  ...
  - Uses a lightweight 8px nav-grid A* pathfinding pass with cached full-path waypoints as the primary movement route, rather than first attempting direct straight-line walking. It falls back to older waypoint avoidance, learned local action scoring, and collision sliding when a route is blocked or ineffective. Interactive furniture and placed items expose generated interaction standpoints so the avatar targets reachable positions near desks, tables, fridges, file cabinets, Coffee Machines, terminals, and other interactable objects rather than trying to stand on the object itself.
