...
- `src/App.tsx`
  ...
  - Furniture skin passes currently include Industrial Bed Skin for the base bed and Industrial Desk Skin for the base desk. Purchased skins can be applied without entering placement mode and do not affect furniture placement, collision, pathfinding, or interaction targets.
...
- `src/game/renderScene.ts`
  ...
  - Renders the current programmatic pixel art pass, including the Stardew-inspired bed, retro drawer desk with an optional Industrial Desk Skin, placed CRT-style Terminal with animated keyboard, metal reflective dining table, retro green two-door fridge with deeper top clutter, buyable File Cabinet, premium black/gray Coffee Machine, Coffee Cup, Switch-style Game Console, Oil Easel, Digital Wall Clock, rainbow Cozy Rug, purple morph-blob rug, fridge door open/hold/close animation, and blanket overlay used when the avatar sleeps under the covers.
  - The bed renderer supports `skinId`. The default bed keeps the warm wood frame, blue star blanket, pillows, sheet, and plush toy. `industrial-bed-skin` swaps in a metal industrial frame, light gray pillows/sheet, and a deeper dark-gray blanket with smoother horizontal pixel shading while preserving the plush toy.
  - The desk renderer supports `skinId`. The default desk keeps the retro drawer/writing-pad look. `industrial-desk-skin` preserves the base desk dimensions, placement, collision, and interaction geometry while swapping in a dark oak desktop, no desk pad, smoother low-contrast wood grain, black metal four-corner legs, tabletop-over-leg occlusion, aligned underside shadow, and a subtle low-frequency pair of yellow eyes that open briefly in the desk shadow and then close.
...
- `public/config/aivatar.config.json`
  ...
  - Furniture skin shop content currently includes Industrial Bed Skin, priced at `240` bits, with `tags: ["furniture-skin", "bed"]` and `targetFurnitureId: "bed"`, and Industrial Desk Skin, priced at `280` bits, with `tags: ["furniture-skin", "desk"]` and `targetFurnitureId: "desk"`. Runtime content is duplicated in `public/config/aivatar.config.json` and the `src/data/defaultContent.ts` fallback.
...
- Furniture Skin shop category:
  - Implemented skins currently include Industrial Bed Skin for the base bed and Industrial Desk Skin for the base desk.
...
- Default new saves start with too few bits to buy Industrial Bed Skin. Use an existing save with enough bits or Debug/Add supplies before testing the purchase and apply flow.
...
- Existing saves may also preserve furniture skin purchase/application state. If a furniture skin does not appear, verify that the active preview port points to the intended worktree and that the save at that origin has purchased and applied the skin.
...
6. Expand the furniture skin system beyond Bed and Desk to Dining Table, Fridge, and File Cabinet, keeping skins visual-only unless a future skin explicitly changes dimensions/collision. Add screenshot/runtime QA for Furniture Skins: shop category visibility, purchased/apply/applied states, `activeFurnitureSkinIds` save/load, default bed/desk fallback, Industrial Bed Skin normal rendering, Industrial Desk Skin normal rendering including table-leg perspective and shadow-eye timing, and sleep blanket overlay.
