...
- Park / hilltop side room
  ...
  - `src/park/ParkApp.tsx` owns the park React shell, canvas loop, cross-window save synchronization, time preview, non-persistent DEBUG controls, developer-window launch, active-character transfer, sea-cliff ambience, grass footsteps, and fishing SFX lifecycle. The main park renderer targets `30 FPS`, keeps simulation timing independent from display refresh rate, and skips canvas rendering while the document is hidden. `src/park/ParkDeveloperApp.tsx` owns placement editing for trees, flowers, grass, rocks, and other park objects; park-editor layout data stays separate from the main-room layout.
  ...
  - `src/park/parkAmbientAudio.ts` owns the Hilltop Park's looping sea-cliff ambience at `public/audio/park-sea-cliff-ambience.ogg`. It uses `0.22` of the same global SFX volume, attempts autoplay on park entry, retries after the first in-park pointer or keyboard interaction if browser policy blocks it, reacts to cross-window volume changes, pauses while the park document is hidden, resumes when visible, and releases the media element on unmount. Keep this ambience quieter than the fishing one-shots.
  - `src/park/parkFootstepAudio.ts` owns four alternating grass-step one-shots at `public/audio/park-grass-step-{1,2,3,4}.wav`. Playback is distance-driven only while a grounded character moves across grass, uses a maximum per-step volume multiplier of `0.07`, and is intentionally disabled for the `cute-ghost` appearance.
