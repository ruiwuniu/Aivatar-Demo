...
- The UI skins are currently CSS/canvas theme layers rather than a full design-token system. New UI components need explicit Classic/Terminal/Amber/Arcade Cabinet theme QA so Classic Windows 98 controls, Terminal/Amber colors, Arcade cabinet/CRT framing, the Arcade scene-panel top bright strip, expanded panels, custom progress bars, disabled states, Decor thumbnails, checkboxes, range sliders, and canvas overlays/bubbles stay coherent.
