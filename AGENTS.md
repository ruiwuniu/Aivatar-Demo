...
- `src/App.tsx`
  ...
  - Audio unlock now only records that a user gesture has occurred; it no longer muted-plays and pauses the shared runtime `Audio` objects, so the Terminal keyboard loop is not accidentally stopped by the unlock flow.
...
- Global SFX volume control in the side panel, persisted per browser/Tauri origin under `aivatar.audioVolume.v1`.
