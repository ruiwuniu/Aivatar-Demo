...
struct UpdaterContext {
    config: Config,
    network_limits: Option<crate::NetworkLimits>,
...
impl UpdaterBuilder {
    ...
    /// Apply bounded streamed reads and HTTPS redirect host restrictions.
    pub fn network_limits(mut self, limits: crate::NetworkLimits) -> Self {
        self.context.network_limits = Some(limits);
        self
    }
...
impl Updater {
    pub async fn check(&self) -> Result<Option<Update>> {
        ...
        for url in &self.endpoints {
            ...
            if let Some(ref limits) = self.context.network_limits {
                request = limits.client(request, &url)?;
            }
                ...
                Ok(res) => {
                    if res.status().is_success() {
                        ...
                        let update_response: serde_json::Value =
                            if let Some(ref limits) = self.context.network_limits {
                                let bytes = crate::network_limits::read_bounded(
                                    res,
...
impl Update {
    ...
    pub async fn download<C: FnMut(usize, Option<u64>), D: FnOnce()>(
        ...
        if let Some(ref limits) = self.context.network_limits {
            request = limits.client(request, &self.download_url)?;
        }
        let response = request
            .build()?
        ...
        let buffer = if let Some(ref limits) = self.context.network_limits {
            crate::network_limits::read_bounded(response, limits.max_download_bytes, on_chunk)
                .await?
        } else {
...
#[cfg(test)]
mod tests {
    use super::{signed_version, verify_signature, verify_signed_version};
    ...
    #[test]
    fn authenticates_artifact_and_version_with_real_minisign_verification() {
        use base64::Engine;
        let fixture: serde_json::Value =
            serde_json::from_str(include_str!("testdata/synthetic-signatures.json")).unwrap();
        let payload = fixture["payload"].as_str().unwrap().as_bytes();
        let key = fixture["publicKey"].as_str().unwrap();
        let signature = fixture["versionedSignature"].as_str().unwrap();
        let legacy = fixture["legacySignature"].as_str().unwrap();
        assert!(verify_signature(payload, signature, key, "1.2.3", true).is_ok());
        assert!(matches!(
            verify_signature(payload, signature, key, "9.9.9", true),
            Err(Error::SignedVersionMismatch { .. })
        ));
        assert!(matches!(
            verify_signature(payload, legacy, key, "1.2.3", true),
            Err(Error::MissingSignedVersion)
        ));
        assert!(verify_signature(b"tampered payload", signature, key, "1.2.3", true).is_err());
        ...
        let forged = base64::engine::general_purpose::STANDARD
            .encode(decoded.replace("version:1.2.3", "version:9.9.9"));
        // Altering the trusted comment must fail authentication, before the
        // announced version can be accepted just because its text now matches.
        assert!(verify_signature(payload, &forged, key, "9.9.9", true).is_err());
    }
